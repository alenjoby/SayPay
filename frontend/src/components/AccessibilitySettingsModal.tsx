...
export const AccessibilitySettingsModal: React.FC<AccessibilitySettingsModalProps> = ({
  ...
  return (
    ...
    >
      <div className="w-full max-w-xl bg-white rounded-3xl p-6 sm:p-7 shadow-2xl border border-slate-200 flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 id="accessibility-settings-title" className="text-xl font-black text-slate-900">
        ...
        <div className="space-y-4 overflow-y-auto pr-1 flex-1 py-4 text-xs">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3.5 rounded-2xl border border-slate-200 bg-white hover:border-slate-300 transition">
              <div>
          ...
          {/* Group 2: Touch & Visual Contrast */}
          <div className="space-y-3 pt-2">
            <h3 className="font-extrabold text-slate-900 text-xs uppercase tracking-wider flex items-center gap-1.5 text-slate-500">
              <Eye className="w-3.5 h-3.5 text-[#00A850]" />
            ...
            <div className="flex items-center justify-between p-3.5 rounded-2xl border border-slate-200 bg-white hover:border-slate-300 transition">
              <div>
                <span className="font-extrabold text-sm text-slate-900 block">
                  High-Contrast Contrast Borders
                </span>
                <span className="text-slate-500 text-[11px] block mt-0.5">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-center transition ${
                      local.fontSize === s.id
                        ? 'border-[#00E575] bg-emerald-50 text-slate-950 font-black shadow-sm'
                        : 'border-slate-200 bg-slate-50 text-slate-600'
                    }`}
                  >
                    <span className="block text-xs font-bold">{s.label}</span>
                    <span className="block text-[10px] text-slate-400 font-mono">{s.desc}</span>
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            >
              <Mic className="w-3.5 h-3.5 text-[#00A850]" />
              <span>Test Audio</span>
            </button>

            <button
              onClick={handleResetDefaults}
              className="px-3 py-2 rounded-xl text-slate-500 hover:text-slate-800 text-xs font-semibold transition flex items-center gap-1"
            >
          ...
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl btn-lime text-slate-950 text-xs font-black transition shadow-sm"
          >
            Apply & Close
          </button>
        </div>
