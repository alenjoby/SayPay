"""CPU runtime for the fine-tuned transformer (int8 ONNX), and the v3 + transformer ensemble.
...
class OnnxIntentModel:
    ...
    def load(model_dir: Path) -> "OnnxIntentModel | None":
        if not (Path(model_dir) / "model.int8.onnx").exists():
            return None
        return OnnxIntentModel(model_dir)
